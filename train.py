...
if arg.kl_type == 'monotonic' : 
    annealer = monotonic_annealer(arg.epochs, arg.kl_start, arg.kl_w_start, arg.kl_w_end)
...
for epoch in range(arg.epochs) : 
    ...
    if not os.path.exists(f'checkpoint/{arg.checkpoint_name}') : 
        os.makedirs(f'checkpoint/{arg.checkpoint_name}')
    torch.save(model.state_dict(), f'checkpoint/{arg.checkpoint_name}/e{epoch+1}_{arg.checkpoint_name}.pth')
    ...
    with torch.no_grad() : 
        ...
        with open(f'genmol_train/{cur_time}.txt', 'a') as file : 
            if epoch == 0 : 
    ...
    writer.add_scalar('Loss/KL Loss', kl_loss / len(train_loader), epoch)
    writer.add_scalar('Loss/Recon Loss', recon_loss / len(train_loader), epoch)
    writer.add_scalar('Loss/Beta', beta, epoch)
